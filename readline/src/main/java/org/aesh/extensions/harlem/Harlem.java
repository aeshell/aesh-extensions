...
public class Harlem implements Completion {
    ...
    private void displayQuestion() throws IOException {
        connection.write(ANSI.START+rows+";1H");
        connection.write("Allow harlem to save file to \""+Config.getTmpDir()+"? (y or n)");
        //processInvocation(..)
    }

    protected void afterDetach() throws IOException {
        connection.write(ANSI.MAIN_BUFFER);
        connection.write(ANSI.START+"?25h");
        attached = false;
    }

    private void displayWait() throws IOException {
        connection.write(ANSI.START+"?25l");
        connection.write(ANSI.START+rows/2+";1H");
        connection.write("Buffering....  please wait.....");
    }

    private void displayIntro() throws IOException {
        connection.write(ANSI.START + "1;1H");
        ...
        for(int i=0; i < 33; i++) {
            ...
            connection.write(ANSI.START+middleRow+";"+middleColumn+"H");
            connection.write(middleChar.toString());
